...
def generate_network(network, num_of_nodes):
	...
	while count > 0:
		for i, row in enumerate(network):
			for j, value in enumerate(row):
				# If the edge is not present in the original graph at time t+1 and has not been added to the new edges yet
	...
	# Return the generated network of time T and the new edges(YES & NO) which will be added to network at time T to generate the network at time T+1 
